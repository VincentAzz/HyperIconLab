...
// SVG 渲染 bitmap 缓存：key = "<drawable>.svg:<fgColorHex>"，避免滚动时重复解析渲染
...
fun LawniconsBrowserSheet(
    ...
    FloatingBottomSheet(
        ...
        // Header：居中标题 + 左侧关闭按钮（与 MaskPickerSheet 风格一致）
...
/**
 * 单个 SVG 图标渲染项。带 LruCache 缓存，避免滚动时重复解析。
 */
@Composable
private fun SvgIconItem(
    ...
    val cacheKey = remember(svgFile, fgColorHex) { "${svgFile.name}:$fgColorHex" }
    ...
    Box(modifier = modifier, contentAlignment = Alignment.Center) {
        Crossfade(targetState = bitmap, label = "icon_fade") { currentBitmap ->
